...
func (s *Store) PrincipalSummaries(ctx context.Context) ([]store.PrincipalSummary, error) {
	const q = `
		...
		-- Runs that have spent more than their budget allows. Nothing enforces
		-- that yet, so this is precisely the "what would have been blocked"
		-- signal observe mode is supposed to surface. A budget of zero means
		-- unset rather than "no allowance", so it is excluded.
		LEFT JOIN (
